...
public class YugabyteDatastore implements Datastore, ProcessorHandler, KairosMetricReporter,
		ServiceKeyStore
{
	public static final Logger logger = LoggerFactory.getLogger(YugabyteDatastore.class);
	...
	private final YugabyteDBModule.CQLBatchFactory m_cqlBatchFactory;
	...
	private final YugabyteDBModule.BatchHandlerFactory m_batchHandlerFactory;
	private final YugabyteDBModule.DeleteBatchHandlerFactory m_deleteBatchHandlerFactory;
	private final YCQLFilteredRowKeyIteratorFactory m_rowKeyFilterFactory;
	...
	public YugabyteDatastore(
			...
			YugabyteDBModule.BatchHandlerFactory batchHandlerFactory,
			YugabyteDBModule.DeleteBatchHandlerFactory deleteBatchHandlerFactory,
			YCQLFilteredRowKeyIteratorFactory rowKeyFilterFactory,
			YugabyteDBModule.CQLBatchFactory cqlBatchFactory
	...
	public Iterator<DataPointsRowKey> getKeysForQueryIterator(DatastoreMetricQuery query) throws DatastoreException
	{
		...
		if (ret == null)
		{
			...
			{
				Object obj = m_rowKeyFilterFactory.create(m_writeCluster, query.getName(), query.getStartTime(),
						query.getEndTime(), query.getTags());
				retList.add((Iterator<DataPointsRowKey>) obj);
			}
			...
			for (ClusterConnection cluster : m_readClusters)
			{
				...
				{
					Object obj = m_rowKeyFilterFactory.create(cluster, query.getName(), query.getStartTime(),
							query.getEndTime(), query.getTags());
					retList.add((Iterator<DataPointsRowKey>) obj);
				}
			}
